function Block( dom ){
	this.dom = dom; //dom
	this.parentW = this.dom.parentNode.clientWidth;//大盒子的宽高
	this.parentH = this.dom.parentNode.clientHeight;
	this.scale = 1.58; //黑块 宽高比
	this.h = parseInt(this.parentW/4*this.scale);//cell：小黑快的高度 ，经过计算这里相当于四分之一parentH
	this.top = -this.h;
	this.speed = 2; //速度
...
//在每一行添加四个cell，随机给一个cell添加样式
	...
	if( fChild == null ){
		this.dom.appendChild(oRow);//起始为空，添加一行
	}else{
		this.dom.insertBefore(oRow , fChild);//在第一个孩子节点上添加一行
	}
}
